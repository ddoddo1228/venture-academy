...
public class Application {
    public static void main(String[] args) {
        ...
        * 현실 세계와 닮아있지만 현실 세계와 다른 점은 객체간의 상호작용을 한다는 사실은
        * 현실 세계와 돌리하지만 의인화 기법이 적용되어 현실세계에서 불가능한 무생물이나 개념 같은 존재들도
        * 하나의 주체로 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 현실 세계와는 다른 점이다.
        ...
        * 상호작용에 중요한 것은 메세지를 송수신하고 그 방법으로 기술하여 메세지에 응답하는 상호작용을 설계하는 것이다.
        * 따라서 객체를 설계할 때는 속성은 부가적이며 적절한 책임을 가진 객체들간의 주고 받는 메세지를 이용해 상호작용하는 것을
        * 설계하는 것을 의미한다.
        * 즉, 필드보다는 메소드를 중점으로 추상화기법을 적용하여 객체를 설계하고,
        * 그 객체를 구현하는 방법으로 설계한대로 클래스를 작성하는 것이다. 이것을 추상화라고 한다.
        * */

        /* 추상화 기법을 이용해서 객체를 설게하고 이를 바탕으로 클래스를 작성한 뒤 객체를 상호작용 시켜보자 */
        ...
        * 5. 카레이서가 엑셀레이터를 밟으면 시동이 걸린 상태일 경우 자동차는 10km/h 증가하며 앞으로 나아간다.
        * 6. 자동차가 달리는 중인 경우 브레이크를 밟으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        * 7. 브레이크를 밟을 떄 자동차가 달리는 중이 아니라면 이미 멈춰있는 상태라고 안내한다.
