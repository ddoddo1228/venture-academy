...
public class C_ifElseIf {
    ...
    public void testNestedIfElseIfStatement() {
        ...
        /* if-else-if문에서 또 다른 조건을 사용하여 if or if-else-if문을 사용할 수 있다. */

        /* ohgiraffers의 대학의 김ㅇㅇ교수는 학생들 시험 성적을 수기로 계산해서 학점 등급을 매기는 채점방식을 사용하고 있었다.
         * 90점 이상이면 'A', 80점 이상이면 'B', 70점 이상이면 'C', 60점 이상이면 'D'를
         * 60점 미만이면 'F'를 학점 등급으로 하는 기준이다.
         * 추가로 각 등급의 중간점수 (95, 85, 75...)인 경우 '+'를 붙여서 등급을 세분화 하다 보니 신경쓸게 이만 저만이 아니다.
        ...
        if (point >= 90) {
            ...
            if (point >= 95) {
                ...
                /* 95점 이상인 경우에만 A등급에 +기호를 덧붙여서 A+ 등급을 만들어준다. */
                grade += "+";
            ...
            /* 점수가 80점 이상인 경우 */
            grade = "B";
            /* 등급은 B등급인데 그럼 +가 붙을 자격이 있는(80점 이상인지) 다시 확인 */
            if (point >= 85) {

                /* 95점 이상인 경우에만 B등급에 +기호를 덧붙여서 B+ 등급을 만들어준다. */
                grade += "+";
            ...
            /* 등급은 C등급인데 그럼 +가 붙을 자격이 있는(70점 이상인지) 다시 확인 */
            if (point >= 75) {

                /* 95점 이상인 경우에만 C등급에 +기호를 덧붙여서 C+ 등급을 만들어준다. */
                grade += "+";
                ...
                /* 등급은 D등급인데 그럼 +가 붙을 자격이 있는(60점 이상인지) 다시 확인 */
                if (point >= 65) {

                    /* 95점 이상인 경우에만 D등급에 +기호를 덧붙여서 D+ 등급을 만들어준다. */
                    grade += "+";

                } else {
                    /* 위 조건을 다 만족하지 못하는 점수가 60점 미만인 경우 */
                    grade = "F";
                    /* 등급이 F인 경우에는 +를 붙이지 않는다. */
                }

                /* 위에서 결정한 등급과 점수를 함께 출력한다. */
